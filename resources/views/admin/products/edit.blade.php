...
@section('content')
    <div class="max-w-4xl mx-auto bg-gray-200 p-6 rounded-md shadow-md">
        <h2 class="text-2xl font-semibold mb-6">Edit Product: {{ $product->name }}</h2>
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                @if (is_array($product->images))
                    <div class="grid grid-cols-3 gap-4 mb-2">
                        @foreach ($product->images as $image)
                            <img src="{{ Storage::url($image) }}" alt="Product Image" class="w-full h-32 object-cover rounded-md">
                        @endforeach
                    </div>
                @endif
                ...
                <p class="text-sm text-gray-500 mt-1">Upload new images to replace existing ones (JPEG, PNG, JPG, GIF, WebP, max 2MB each).</p>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">-surroundWidth (cm)</label>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="color" value="{{ old('color', $product->attributes['color'] ?? '') }}"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Size</label>
                    <input type="text" name="size" value="{{ old('size', $product->attributes['size'] ?? '') }}"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                </div>
            </div>
                    ...
                    @foreach ($categories as $category)
                        <div class="form-check">
                            <input
                                ...
                                {{ $product->categories->contains($category) ? 'checked' : '' }} <!-- Check if the category is selected -->
                            >
