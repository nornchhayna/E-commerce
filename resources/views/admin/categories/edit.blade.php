...
<div class="container mx-auto p-6">
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow-md">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" required class="w-full border px-4 py-2 rounded">
        </div>

        <div class="mb-4">
            <label class="block font-semibold mb-1" for="slug">Slug</label>
            <input type="text" name="slug" id="slug" value="{{ old('slug', $category->slug) }}" class="w-full border px-4 py-2 rounded">
        </div>

        <div class="mb-4">
            <label class="block font-semibold mb-1" for="description">Description</label>
            <textarea name="description" id="description" rows="4" class="w-full border px-4 py-2 rounded">{{ old('description', $category->description) }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="parent_id" id="parent_id" class="w-full border px-4 py-2 rounded">
                ...
                @foreach ($categories as $parent)
                    @if ($parent->id !== $category->id) {{-- prevent selecting itself --}}
        ...
        <div class="mb-4">
            ...
            <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $category->sort_order) }}" class="w-full border px-4 py-2 rounded">
        </div>
